...
setwd("C:/Users/MIQ/Desktop/CMU Docs/Spring 22/45936 - Marketing Analytics/Week 1/HW1 - Ford Ka Case") # !!! change this line !!!
...
nshortqlist=c(29,1,4,12)  # short list of questions
...
CrossTable(ford$PreferenceGroup,ford$Q1) 
...
plot(kclust,wssB/bssB,type="l",main="Ratio of within cluster to between cluster variance")
points(kclust,wssB/bssB)
...
# create tables to describe the data
xtabs(~PreferenceGroup,data=xford)
xtabs(~Q1,data=xford)

# to see the relationship between two variables do a cross-tab
xtabs(~PreferenceGroup+Q1,data=xford)
# here is a more visualize representation of a table with a BalloonPlot (uncomment the line if you want to see it)
balloonplot(table(ford$PreferenceGroup,ford$Q1),xlab="PreferenceGroup",ylab="Q1")
...
# compute multiple cluster solutions
grpA2=kmeans(xfordpsyc[,qlist],centers=2)
grpA3=kmeans(xfordpsyc[,qlist],centers=3)
grpA4=kmeans(xfordpsyc[,qlist],centers=4)
grpA5=kmeans(xfordpsyc[,qlist],centers=5)
grpA6=kmeans(xfordpsyc[,qlist],centers=6)
grpA7=kmeans(xfordpsyc[,qlist],centers=7)
grpA8=kmeans(xfordpsyc[,qlist],centers=8)
grpA9=kmeans(xfordpsyc[,qlist],centers=9)
grpA10=kmeans(xfordpsyc[,qlist],centers=10)
grpA15=kmeans(xfordpsyc[,qlist],centers=15)
grpA20=kmeans(xfordpsyc[,qlist],centers=20)
grpA30=kmeans(xfordpsyc[,qlist],centers=30)
...
(grpA=kmeans(xfordpsyc[,qlist],centers=k))

# plot the solutions against the Q1 and Q2
# since the data is categorical most of the plots will overlay one another,
# so instead we jitter the points -- which adds a small random number to each
par(mfrow=c(1,1),mar=c(5,4,4,1)+.1)
plot(jitter(xfordpsyc[,"Q46"]), jitter(xfordpsyc[,"62"]),xlab=fordquest[46],ylab=fordquest[62],col=grpA$cluster)
points(grpA$centers[,c("Q46","Q62")],col=1:k,pch=8,cex=2)
